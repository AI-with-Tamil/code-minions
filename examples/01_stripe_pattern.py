...
Key details from Stripe's published architecture:
...
async def gather_context(ctx: RunContext) -> None:
    """
    Runs before the agent starts. Hydrates context from the task.
    Stripe does this deterministically over links and tickets in the task.
    Keeps agent context focused — don't let the agent waste tokens fetching basics.
    """
    result = await ctx.exec("git log --oneline -10")
    ctx.state.context_summary = result.stdout
    # In production: resolve links in ctx.task, fetch ticket body, run MCP tools
    # over any URLs. Results go into ctx.state so the agent has them ready.
