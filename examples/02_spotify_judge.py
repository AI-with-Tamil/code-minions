...
Pattern:
    implement → judge → (veto → retry implement)? → lint → test → push

Key insight from Spotify:
- The judge doesn't fix the code — it only evaluates and rejects
- Agent self-corrects when given the veto reason
- Most common veto: scope creep, unrelated changes, going outside instructions
...
async def push_and_pr(ctx: RunContext) -> None:
    ...
    await ctx.exec(
        ...
        f'--body "Automated by Minion SDK" --head {ctx.state.branch}'
    )
...
spotify_blueprint = Blueprint(
    ...
    nodes=[
        ...
        AgentNode(
            "implement",
            system_prompt=(
                "Complete the task. Stay focused — only change what the task requires. "
        ...
        JudgeNode(
            ...
            evaluates="implement",
            criteria=(
                "The change matches the task description exactly. "
