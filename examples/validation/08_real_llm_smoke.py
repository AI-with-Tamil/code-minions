...
Example type:
- Validation example
- Not a company-pattern example
- Exists to prove the real tool loop and acceptance path work end to end
...
from codeminions import AgentNode, Blueprint, DeterministicNode, Minion, RunContext, Task
...
def _init_git_repo(root: Path) -> None:
    ...
    _run(["git", "config", "user.name", "CodeMinions Smoke"], cwd=root)
    _run(["git", "config", "user.email", "codeminions-smoke@example.com"], cwd=root)
...
async def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="codeminions-smoke-") as tmp:
        repo = Path(tmp)
        _write_repo(repo)
        _init_git_repo(repo)
