...
from codeminions.core.task import Task, coerce_task
from codeminions.events import MinionEvent
from codeminions.trace import Trace, TraceEvent
...
class Minion:
    ...
    def __init__(
        ...
        minion_config = _load_toml_table(project_root / "codeminions.toml", "codeminions")
        pyproject_config = _load_toml_table(project_root / "pyproject.toml", "tool", "codeminions")

        self._model_spec = self._resolve_spec(
            explicit=model,
            default_string=_MODEL_DEFAULT,
            minion_value=minion_config.get("model"),
            pyproject_value=pyproject_config.get("model"),
            env_var="CODEMINIONS_MODEL",
            fallback=_MODEL_DEFAULT,
        )
        self._blueprint_spec = self._resolve_spec(
            explicit=blueprint,
            default_string=_BLUEPRINT_DEFAULT,
            minion_value=minion_config.get("blueprint"),
            pyproject_value=pyproject_config.get("blueprint"),
            env_var="CODEMINIONS_BLUEPRINT",
            fallback=_BLUEPRINT_DEFAULT,
        )
        self._environment_spec = self._resolve_spec(
            explicit=environment,
            default_string=_ENVIRONMENT_DEFAULT,
            minion_value=minion_config.get("environment"),
            pyproject_value=pyproject_config.get("environment"),
            env_var="CODEMINIONS_ENVIRONMENT",
